...
infilename = "out_commonwordcounts_table_fraclength.txt"
...
def load_table( filename ):
	...
	return rh, ch, contents[1:,1:].astype( numpy.float32 )


def do_svd( dataset , usecounts = False ):
	# fullmatrices= false is the compact SVD transformation, returning a limited number of basis vectors (8, not 438)
	# We can perform SVD based on how many times a feature appears (usecounts = True), or just whether it appears at all
	if not usecounts:
		dataset[ dataset > 0 ] = 1
	u, s, v = numpy.linalg.svd( dataset , full_matrices = True )
	return u, s ,v
...
if __name__ == "__main__":
	...
	u, s, v = do_svd( data, usecounts = True )
